...
import { CreateReservationInput, UpdateReservationStatusInput } from "./reservation.schema";
...
export const createReservation = async (data: CreateReservationInput) => {
  ...
  if (data.type === "POOL" || data.type === "BOTH") {
    ...
    if (poolConflict) throw new AppError("Pool slot is already reserved", 409);
  }
